...
#endif
CTFAnnotationsPanel *g_pAnnotationsPanel = NULL;
...
//-----------------------------------------------------------------------------
bool FormatTrainingText(const char* inputString, wchar_t* outputString)
{
...
{
	SetAlpha(0); // Gets rid of box on the top left
...
CTFAnnotationsPanelCallout *CTFAnnotationsPanel::TestAndAddCallout( int id, Vector &origin, const char *text )
{
	...
	for (int i=0; i<m_pCalloutPanels.Count(); ++i)
	{
		...
		{
#if 0
			m_pCalloutPanels[i]->DeletePanel();
#else
			...
			m_pCalloutPanels[i]->m_flAlpha[0] = m_pCalloutPanels[i]->m_flAlpha[1] = 0.0f;
...
ConVar cl_portal_coop_show_annotations("cl_portal_coop_show_annotations", "1", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE, "Sets if you can see your own alerts when pinging");
ConVar cl_portal_coop_show_my_annotations("cl_portal_coop_show_my_annotations", "1", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE, "Sets if you can see your own alerts when pinging");
...
void CTFAnnotationsPanel::AddAnnotation( IGameEvent * event )
{
	...
	int iOwnerEntIndex = event->GetInt( "owner_entindex" );
	...
	int iForcedPingIcon = event->GetInt( "forcedpingicon" );

	if ( ( !cl_portal_coop_show_my_annotations.GetBool() && ClientEntityList().GetEnt(iOwnerEntIndex) == C_BasePlayer::GetLocalPlayer() ||
		!cl_portal_coop_show_annotations.GetBool() ) 
		)
	{
		return;
	}
	...
	if ( pCallout )
	{
		...
		pCallout->SetOwner( ClientEntityList().GetEnt(iOwnerEntIndex) );
		pCallout->SetForcedPingIcon( iForcedPingIcon );
...
CTFAnnotationsPanelCallout::~CTFAnnotationsPanelCallout()
{
	...
	if (m_pAnnotationImage)
	{
		m_pAnnotationImage->DeletePanel();
	}
}
...
void CTFAnnotationsPanelCallout::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	if ( m_pBackground )
	{
		...
		//vgui::Panel *pParent = g_pClientMode->GetViewport();
		m_pAnnotationImage->SetParent( this );
		//m_pAnnotationImage->SetParent( m_pBackground );
	...
	if ( m_pAnnotationLabel && FormatTrainingText( m_Text, outputText ) )
	{
...
void CTFAnnotationsPanelCallout::PerformLayout( void )
{
	...
	if (m_DeathTime > 0.0f && m_DeathTime <= gpGlobals->curtime)
	{
		//g_pAnnotationsPanel->HideAnnotation( GetAnnotationID() );
		//SetVisible(false);
		DeletePanel();
		if (m_pAnnotationImage)
		m_pAnnotationImage->DeletePanel();
	...
	if ( !m_pAnnotationLabel || !m_pDistanceLabel || !m_pAnnotationImage )
		return;

	C_Portal_Player *pLocalPortalPlayer = C_Portal_Player::GetLocalPortalPlayer();
	...
	m_pAnnotationImage->SetDrawColor(color);
	...
	if ( m_FollowEntity.Get() )
	{
		...
		{
			// NPCs are weird, so we do this:
			if(m_FollowEntity->IsNPC())
				vecTarget.z += (m_FollowEntity->CollisionProp()->OBBSize().z);
			else
				vecTarget.z += (m_FollowEntity->CollisionProp()->OBBSize().z) - (m_FollowEntity->CollisionProp()->OBBSize().z * 0.5);
				
			
		}
		else if ( m_FollowEntity->IsNPC() )
		{
		//	vecTarget.z += (m_FollowEntity->CollisionProp()->OBBSize().z) - (m_FollowEntity->CollisionProp()->OBBSize().z * 0.5);
		}
	}
	...
	*/
	{
		m_flAlpha[1] = 255;
		SetBgColor(Color(0, 0, 0, 0.0));
		SetFgColor(Color(0, 0, 0, 0.0));
	...
	//SetAlpha( fade_alpha ); // Setting this to 0 will make everything invisible
	SetBgColor( Color( 0, 0, 0, 0 ) ); // Gets rid of the random display
	m_pArrow->SetAlpha( 255.0 );
	m_pBackground->SetAlpha( 0 ); // Setting this to 0 clears the white, but also clears the text and the image (not that the text matters in portalcoop)


	m_pAnnotationImage->SetAlpha( 255.0 );
	//m_pAnnotationImage->SetPos( m_pBackground->GetXPos(), m_pBackground->GetYPos() ); //HACK!! Let m_pBackground have 0 alpha while keeping the Annotating Image in the same location without needing a parent!


	m_pBackground->SetBgColor( Color( 0, 0, 0, 0) );
	m_pBackground->SetFgColor( Color( 0, 0, 0, 0) );

	const int halfWidth = m_pBackground->GetWide() / 2;
	bool bOffscreen = !bOnscreen || iX < halfWidth || iX > ScreenWidth()-halfWidth;
	if ( bOffscreen )
	{
		m_pArrow->SetSize( XRES( 16 ), YRES( 20 ) );
	...
	else
	{
		...
		if ( tr.fraction < 1.0f )
		{
			...
			// This isn't a problem in portalcoop
			/*
			if ( m_FollowEntity.Get() &&
				 m_FollowEntity->GetTeamNumber() != TEAM_UNASSIGNED && m_FollowEntity->GetTeamNumber() != TEAM_INVALID &&
				 m_FollowEntity->GetTeamNumber() != pLocalPortalPlayer->GetTeamNumber() && 
				 pLocalPortalPlayer->GetTeamNumber() != TEAM_SPECTATOR )
			{
				SetAlpha( 0 );
				m_pArrow->SetAlpha( 0 );
			}
...
//	int wide2 = wide;
	...
	if ( m_pBackground )
	{
		...
		m_pAnnotationImage->SetSize( XRES(20), YRES(27) );
	...
	if ( bOffscreen )
	{
		...
		if ( m_pBackground )
		{
			if ( iX + m_pBackground->GetWide() / 2 < ScreenWidth() / 2 )
			{
				...
				//God this is so frustrating to work with - Wonderland_War

				m_pAnnotationImage->SetPos((GetWide() - m_pAnnotationImage->GetWide()) / 2.333, (m_pBackground->GetTall() - m_pAnnotationImage->GetTall() / 2) - YRES(12));
			}
			else
			{
				...
				m_pAnnotationImage->SetPos((m_pBackground->GetWide() - m_pAnnotationImage->GetWide()) / 2, (m_pBackground->GetTall() - m_pAnnotationImage->GetTall() / 2) - YRES(12));
			}
		}
	}
	else
	{
		if ( m_pBackground )
		{
			...
			//Having the arrow point down towards the obvious target is unnecessary
#if 0
			...
			m_pAnnotationImage->SetPos((m_pBackground->GetWide() - m_pAnnotationImage->GetWide()) / 2, (GetTall() - GetTall() / 2) - YRES(8));
		}
	}
...
void CTFAnnotationsPanelCallout::SetupPingImage()
{
	...
	if (!m_FollowEntity)
	{
		m_pAnnotationImage->SetImage("pingicons/default");
		return;
	}
	*/

	// You can only force default if you use an out of range value
	if ( pingicon == PING_ICON_DEFAULT && m_FollowEntity )
		pingicon = m_FollowEntity->GetPingIcon();
	
	if (pingicon == PING_ICON_DEFAULT)
		m_pAnnotationImage->SetImage("pingicons/default");
	else if (pingicon == PING_ICON_DOOR)
		m_pAnnotationImage->SetImage("pingicons/door");
	else if (pingicon == PING_ICON_BOX)
		m_pAnnotationImage->SetImage("pingicons/box");
	else if (pingicon == PING_ICON_SPHERE)
		m_pAnnotationImage->SetImage("pingicons/sphere");
	else if (pingicon == PING_ICON_FLOORBUTTON)
		m_pAnnotationImage->SetImage("pingicons/floor_button");
	else if (pingicon == PING_ICON_SWITCH)
		m_pAnnotationImage->SetImage("pingicons/switch");
	else if (pingicon == PING_ICON_ENERGYBALL_LAUNCHER)
		m_pAnnotationImage->SetImage("pingicons/energyball_launcher");
	else if (pingicon == PING_ICON_ENERGYBALL_CATCHER)
		m_pAnnotationImage->SetImage("pingicons/energyball_catcher");
	else if (pingicon == PING_ICON_PISTON_VERTICAL)
		m_pAnnotationImage->SetImage("pingicons/piston_vertical");
	else if (pingicon == PING_ICON_PISTON_HORIZONTAL)
		m_pAnnotationImage->SetImage("pingicons/piston_horizontal");
	else if (pingicon == PING_ICON_PORTAL)
		m_pAnnotationImage->SetImage("pingicons/portal");
	else if (pingicon == PING_ICON_TURRET)
		m_pAnnotationImage->SetImage("pingicons/turret");
	else if (pingicon == PING_ICON_GLADOS_CORE)
		m_pAnnotationImage->SetImage("pingicons/glados_core");
	else
		m_pAnnotationImage->SetImage("pingicons/default");
...
bool CTFAnnotationsPanelCallout::UpdateCallout()
{
	...
	C_Portal_Player *pLocalPortalPlayer = C_Portal_Player::GetLocalPortalPlayer();
	if ( !pLocalPortalPlayer )
		return true;
	
	if ( m_iVisibilityBitfield != 0 )
	{
		int bit = 1 << pLocalPortalPlayer->entindex();
...
void CTFAnnotationsPanelCallout::SetOwner( C_BaseEntity *pEntity )
{
	m_hOwner = pEntity;
}