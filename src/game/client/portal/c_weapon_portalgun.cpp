...
CLIENTEFFECT_MATERIAL( PORTALGUN_RED_LAST_GLOW )
...
//	RecvPropFloat( RECVINFO( m_fCanPlacePortal2OnThisSurface ) ),
	RecvPropFloat( RECVINFO( m_fCanPlacePortal1OnThisSurfaceNetworked ) ),
	RecvPropFloat( RECVINFO( m_fCanPlacePortal2OnThisSurfaceNetworked ) ),
...
void C_WeaponPortalgun::Spawn( void )
{
	...
	SetThink( &C_WeaponPortalgun::Think );
...
//	if ( m_Parameters[PORTALGUN_PORTAL2LIGHT_WORLD].GetMaterial() == NULL )
	if ( m_iOldPortalLinkageGroupID != m_iPortalLinkageGroupID )
	{
		...
		{
			m_Parameters[PORTALGUN_PORTAL2LIGHT_WORLD].SetMaterial(PORTALGUN_PORTAL2_FIRED_LAST_GLOW);
		}
	}
...
void C_WeaponPortalgun::OpenProngs( bool bOpenProngs )
{
