...
// Only requires active portal, not linked
CProp_Portal* UTIL_Portal_FirstAlongRayAll(const Ray_t &ray, float &fMustBeCloserThan)
{
    CProp_Portal *pIntersectedPortal = NULL;
	...
	else
	{
		...
		if( realTrace.m_pEnt && (realTrace.m_pEnt->GetMoveType() != MOVETYPE_NONE) ) //started by hitting something moving which wouldn't be detected in the following traces
		{
			...
			else
			{
				...
				if ( vFirstPortalForward.Dot( realTrace.endpos - pFirstPortal->m_ptOrigin ) > 0.0f )
					*pTrace = realTrace;
			}
		}
#endif
...
void UTIL_Portal_VectorTransform( const VMatrix matThisToLinked, const vec_t &vSourceX, const vec_t &vSourceY, const vec_t &vSourceZ, vec_t &vTransformedX, vec_t &vTransformedY, vec_t &vTransformedZ )
{
	Vector vSource;
	vSource.x = vSourceX;
	vSource.y = vSourceY;
	...
	vTransformedX = vTransformed.x;
	vTransformedY = vTransformed.y;
	vTransformedZ = vTransformed.z;
}
...
bool UTIL_EntityIsIntersectingPortalWithLinkedAlsoBeingFloorOrCeiling(const CBaseEntity *pEntity)
{
	...
	for( int i = 0; i != iPortalCount; ++i )
	{
		...
		{
			if ( pTempPortal->IsCeilingPortal() && pTempPortal->m_hLinkedPortal->IsCeilingPortal() )
				return true;

			if ( pTempPortal->IsFloorPortal() && pTempPortal->m_hLinkedPortal->IsFloorPortal() )
				return true;
...
CProp_Portal *UTIL_PointIsOnPortalQuad(const Vector vPoint, float fOnPlaneEpsilon, CProp_Portal * const *pPortalsToCheck, int iArraySize)
{
	...
	CProp_Portal **pPlanarCandidates = (CProp_Portal **)stackalloc(sizeof(CProp_Portal *) * iArraySize);
	...
	for( int i = 0; i != iPlanarCandidateCount; ++i )
	{
		...
		if( (fabs( vDiff.Dot( pPlanarCandidates[i]->m_vRight ) ) < PORTAL_HALF_WIDTH) &&
			(fabs( vDiff.Dot( pPlanarCandidates[i]->m_vUp ) ) < PORTAL_HALF_HEIGHT) )
		{
...
bool UTIL_Portal_EntityIsInPortalHole( const CProp_Portal *pPortal, CBaseEntity *pEntity )
{
	...
	Vector ptPortalCenter = pPortal->m_ptOrigin;
