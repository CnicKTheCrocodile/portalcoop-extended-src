...
END_RECV_TABLE()
BEGIN_PREDICTION_DATA_NO_BASE(PS_SimulationData_t)
	DEFINE_PRED_FIELD(hCollisionEntity, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE)
END_PREDICTION_DATA()
...
END_RECV_TABLE()
BEGIN_PREDICTION_DATA_NO_BASE(PS_InternalData_t)
	DEFINE_PRED_FIELD(Simulation, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE)
END_PREDICTION_DATA()
#endif // ifdef GAME_DLL
...
BEGIN_PREDICTION_DATA_NO_BASE(CPortalSimulator)
	DEFINE_PRED_FIELD(m_InternalData, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE),
END_PREDICTION_DATA()
#endif // ifdef GAME_DLL
...
//	GetInternalData()(m_InternalData)
{
	s_PortalSimulators.AddToTail( this );

#ifdef CLIENT_DLL
	m_bGenerateCollision = (GameRules()->IsMultiplayer());
#endif
...
void CPortalSimulator::TakeOwnershipOfEntity( CBaseEntity *pEntity )
{
	...
	if( pEntity->GetServerVehicle() != NULL ) //we don't take kindly to vehicles in these here parts. Their physics controllers currently don't migrate properly and cause a crash
		return;
#else
	if( pEntity->GetClientVehicle() != NULL ) //we don't take kindly to vehicles in these here parts. Their physics controllers currently don't migrate properly and cause a crash
		return;
#endif
...
void CPortalSimulator::CreateLocalPhysics( void )
{
...
#else
		params.pGameData = GetClientWorldEntity();
#endif
	...
	//World
	{
		...
		if( m_InternalData.Simulation.Static.World.Brushes.pCollideable != NULL )
		{
			...
			if( (m_InternalData.Simulation.hCollisionEntity != NULL) && (m_InternalData.Simulation.hCollisionEntity->VPhysicsGetObject() == NULL) )
...
void CPortalSimulator::CreateLocalCollision( void )
{
	...
	if ( portal_clone_displacements.GetBool() )
	{
		...
		displacementRejectRegions[1].m_Dist = displacementRejectRegions[1].m_Normal.Dot( m_InternalData.Placement.ptCenter ) + sv_portal_collision_sim_bounds_x.GetFloat();
		displacementRejectRegions[2].m_Normal = m_InternalData.Placement.vRight;
		displacementRejectRegions[2].m_Dist = displacementRejectRegions[2].m_Normal.Dot( m_InternalData.Placement.ptCenter ) + sv_portal_collision_sim_bounds_y.GetFloat();
		displacementRejectRegions[3].m_Normal = -m_InternalData.Placement.vRight;
		displacementRejectRegions[3].m_Dist = displacementRejectRegions[3].m_Normal.Dot( m_InternalData.Placement.ptCenter ) + sv_portal_collision_sim_bounds_y.GetFloat();
		displacementRejectRegions[4].m_Normal = m_InternalData.Placement.vUp;
		displacementRejectRegions[4].m_Dist = displacementRejectRegions[4].m_Normal.Dot( m_InternalData.Placement.ptCenter ) + sv_portal_collision_sim_bounds_z.GetFloat();
		displacementRejectRegions[5].m_Normal = -m_InternalData.Placement.vUp;
		displacementRejectRegions[5].m_Dist = displacementRejectRegions[5].m_Normal.Dot( m_InternalData.Placement.ptCenter ) + sv_portal_collision_sim_bounds_z.GetFloat();
		*/
		...
		CPhysCollide* displacementCollision = enginetrace->GetCollidableFromDisplacementsInAABB( vAABBMins, vAABBMaxs );

		CPhysConvex *pDisplacementConvexes[64];
		
		physcollision->GetConvexesUsedInCollideable( displacementCollision, pDisplacementConvexes, 64 );
		...
		CPhysCollide* displacement = enginetrace->GetCollidableFromDisplacementsInAABB( vAABBMins, vAABBMaxs );
		m_InternalData.Simulation.Static.World.Displacements.pCollideable = displacement;
#endif
	...
	//World
	{
		...
		//static props
		{
			...
			for( int i = StaticProps.Count(); --i >= 0; )
			{
				...
				// Don't recreate non-solid static prop geometry
...
void CPortalSimulator::Pre_UTIL_Remove( CBaseEntity *pEntity )
{
	...
	if(
#ifndef CLIENT_DLL
		(CPhysicsShadowClone::IsShadowClone( pEntity ) == false) &&
#endif
...
public:
	...
	virtual void OnEntityDeleted( CBaseEntity *pEntity )
	{
		...
		IClientEntityListener::OnEntityDeleted(pEntity);
#endif
...
IPhysicsObject *CPSCollisionEntity::VPhysicsGetObject( void )
{
	...
	else if( m_pOwningSimulator->GetInternalData().Simulation.Static.Wall.Local.Tube.pPhysicsObject != NULL )
		return m_pOwningSimulator->GetInternalData().Simulation.Static.Wall.Local.Brushes.pPhysicsObject;
...
int CPSCollisionEntity::VPhysicsGetObjectList( IPhysicsObject **pList, int listMax )
{
	...
	if( m_pOwningSimulator->GetInternalData().Simulation.Static.World.Brushes.pPhysicsObject != NULL )
	{
		pList[iRetVal] = m_pOwningSimulator->GetInternalData().Simulation.Static.World.Brushes.pPhysicsObject;
		++iRetVal;
		if( iRetVal == listMax )
			return iRetVal;
	}

	if( m_pOwningSimulator->GetInternalData().Simulation.Static.Wall.Local.Brushes.pPhysicsObject != NULL )
	{
		pList[iRetVal] = m_pOwningSimulator->GetInternalData().Simulation.Static.Wall.Local.Brushes.pPhysicsObject;
		++iRetVal;
		if( iRetVal == listMax )
			return iRetVal;
	}
	...
	if( m_pOwningSimulator->GetInternalData().Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pPhysicsObject != NULL )
	{
		pList[iRetVal] = m_pOwningSimulator->GetInternalData().Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pPhysicsObject;
		++iRetVal;
		if( iRetVal == listMax )
			return iRetVal;
	}
...
#define PSDAC_INTENSITY_REMOTEPROP 0.5f
	...
	if( pPortalSimulator->GetInternalData().Simulation.Static.World.Brushes.pCollideable )
		PortalSimulatorDumps_DumpCollideToGlView( pPortalSimulator->GetInternalData().Simulation.Static.World.Brushes.pCollideable, vec3_origin, vec3_angle, PSDAC_INTENSITY_LOCALBRUSH, szFileName );
	...
	if( pLinkedPortal )
	{
		...
		if ( pLinkedPortal->GetInternalData().Simulation.Static.World.Displacements.pCollideable )
			PortalSimulatorDumps_DumpCollideToGlView( pLinkedPortal->GetInternalData().Simulation.Static.World.Displacements.pCollideable, pPortalSimulator->GetInternalData().Placement.ptaap_LinkedToThis.ptOriginTransform, pPortalSimulator->GetInternalData().Placement.ptaap_LinkedToThis.qAngleTransform, PSDAC_INTENSITY_REMOTEBRUSH, szFileName );
