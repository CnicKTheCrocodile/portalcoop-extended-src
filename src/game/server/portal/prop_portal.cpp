...
void CProp_Portal::Precache( void )
{
	...
	PrecacheModel("models/portals/portal1_id4.mdl");
	PrecacheModel("models/portals/portal2_id4.mdl");
...
void CProp_Portal::ResetModel( void )
{
	...
	if (iPortalColorSet == PORTAL_COLOR_SET_LIGHTBLUE_PURPLE)
		...
		SetModel((m_bIsPortal2) ? ("models/portals/portal2_id4.mdl") : ("models/portals/portal1_id4.mdl"));
	else
...
void CProp_Portal::DoFizzleEffect( int iEffect, PortalColorSet_t iPortalColorSet, bool bDelayedPos /*= true*/ )
{
		...
		if ( modelinfo->GetModelType( pOther->GetModel() ) == mod_brush )
		{
			if ( !FClassnameIs( pOther, "func_physbox" ) && !FClassnameIs( pOther, "simple_physics_brush" ) )	// except CPhysBox
			{
				...
				if ( UTIL_IsBoxIntersectingPortal( ( vMin + vMax ) / 2.0f, ( vMax - vMin ) / 2.0f - Vector( 2.0f, 2.0f, 2.0f ), this, 0.0f ) )
				{
					...
					DoFizzleEffect( PORTAL_FIZZLE_KILLED, ConvertLinkageIDToColorSet( GetLinkageGroup() ), false );
					Fizzle();
				}
				else
				{
