...
public:
	...
	{
		// We want the rocket turret's eye to be
	...
	: m_filterBeams( NULL, COLLISION_GROUP_DEBRIS )
{
	...
	m_flFieldOfView = -1.0f; // 60 degrees
...
void CNPC_RocketTurret::UpdateAimPoint ( void )
{
	...
	Vector vecMid = GetMuzzlePos();
	Vector vecMidEnemy = pEnemy->GetAbsOrigin() + (pEnemy->WorldAlignMins() + pEnemy->WorldAlignMaxs()) * 0.5f;
	...
	{
		bool bEnemyVisibleInWorld = FVisible(pEnemy);
	...
	if ( !m_bHasSightOfEnemy )
	{
		if ( m_flTimeSpentPaused >= m_flPauseLength )
		{
				 ...
				 SignDiffers( m_flTotalDivergenceX, flOffsetX ) )
			{
				...
				if (!GetEnemy())
				{
					m_vecGoalAngles.x = m_vecGoalAngles.x + flOffsetX;
				}
...
void CNPC_RocketTurret::FollowThink( void )
{
	...
	Vector vEndPoint = GetMuzzlePos() + vForward*ROCKET_TURRET_RANGE;
	rayDmg.Init( GetMuzzlePos(), vEndPoint );
	rayDmg.m_IsRay = true;
	trace_t traceDmg;
...
void CNPC_RocketTurret::FireRocket ( void )
{
	...
	CRocket_Turret_Projectile *pRocket = (CRocket_Turret_Projectile *) CBaseEntity::Create( "rocket_turret_projectile", GetMuzzlePos(), m_vecCurrentAngles, this );
...
void CNPC_RocketTurret::DyingThink( void )
{
	...
	if ( m_flTimeSpentDying > ROCKET_TURRET_DEATH_EFFECT_TIME )
	{
		Vector vForward;
		AngleVectors( m_vecCurrentAngles, &vForward, NULL, NULL );
		g_pEffects->EnergySplash( GetMuzzlePos(), vForward, true );
...
void CNPC_RocketTurret::DeathThink( void )
{
	...
	g_pEffects->Sparks( GetMuzzlePos(), 1, 1, &vForward );
	g_pEffects->Smoke( GetMuzzlePos(), 0, 6.0f, 20 );
...
void CNPC_RocketTurret::OpeningThink()
{
	...
	QAngle vecNeutralAngles = GetAbsAngles() + QAngle( 0, 90, 0 );
...
//-----------------------------------------------------------------------------
void CNPC_RocketTurret::HackFindEnemy(void)
{
...
bool CNPC_RocketTurret::TestLOS( const Vector& vAimPoint )
{
	...
	Vector vecToAimPoint = vAimPoint - GetMuzzlePos();
	VectorAngles( vecToAimPoint, m_vecCurrentAngles.m_Value );
	SyncPoseToAimAngles();

	Vector vFaceOrigin = GetMuzzlePos();
	trace_t trTarget;
	Ray_t ray;
...
bool CNPC_RocketTurret::TestPortalsForLOS( Vector* pOutVec, bool bConsiderNonPortalAimPoint = false )
{
	...
	for( int i = 0; i != iPortalCount; ++i )
	{
		...
		{
			QAngle goalAngles;
			Vector vecToEnemy = portalAimPoints[ i ] - GetMuzzlePos();
	...
	if ( bConsiderNonPortalAimPoint )
	{
		...
		Vector vecToEnemy = vAimPoint - GetMuzzlePos();
		vecToEnemy.NormalizeInPlace();
...
{ 
	if ( pPortal && pPortal->IsActive() )
	{
		...
		VectorSubtract ( pPortal->GetAbsOrigin(), GetMuzzlePos(), vRocketToPortal );
