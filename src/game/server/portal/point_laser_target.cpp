...
{ // Line 192
	...
	if ( pCore )
	{
		...
		{
			dist = 24;
			AngleVectors( qAbsAngles, NULL, NULL, &dir );
			VectorAngles( dir, qAbsAngles );
			pCore->SetScale( 0.75 );
		}

		pCore->SetAbsOrigin( GetAbsOrigin() + ( dir * dist ) );
...
{ // Line 279
    ...
    //StopParticleEffects( this ); // Line 288
...
{ // Line 298
	...
	m_iPowerState = POWERING_UP; // Line 306
} // Line 309
...
{ // Line 316
	...
	m_iPowerState = POWERING_DOWN; // Line 323
} // Line 326
...
{ // Line 357
    if ( !MAKE_STRING( GetModelName().ToCStr() ) )
		return LASER_CATCHER_NAME;
    return GetModelName().ToCStr();
}
...
{ // Line 426
    ...
    if ( m_pCatcherLaserTarget ) // Line 429
    {
        ...
        if ( m_pCatcherLaserTarget->IsPowered() ) // Replaced with optimized function "IsPowered"
        {
            ...
            DispatchParticleEffect( "laser_relay_powered", PATTACH_POINT_FOLLOW, this, "particle_emitter", false ); // Line 438
        }
    }
